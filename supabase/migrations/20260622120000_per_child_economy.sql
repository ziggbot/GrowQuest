-- Per-child economy.
--
-- Moves the economy knobs that used to live family-wide on
-- profile_configs down onto each child, so siblings can run different
-- rules: a 6-year-old and a 13-year-old rarely want the same daily
-- screen-time cap, daily-mission requirement, or mynt expiry.
--
-- profile_configs keeps its columns (the multipliers still live there,
-- and the old values are what we backfill from), but the per-child
-- columns below are now authoritative for the redeem / expiry RPCs.

-- ─── 1. New per-child columns ────────────────────────────────────────
...
-- ─── 2. Backfill from the family config so behaviour is preserved ─────
update public.child_profiles c
set require_daily_mission = pc.require_daily_mission,
    mynt_expiry_days      = pc.mynt_expiry_days
from public.profile_configs pc
where pc.family_id = c.family_id;

-- Screen time is per-child now: give every child that was riding on the
-- family default an explicit override, so "use the family limit" stops
-- being a concept the UI has to expose.
update public.child_profiles c
set daily_limit_minutes_override = pc.daily_limit_minutes
from public.profile_configs pc
where pc.family_id = c.family_id
  and c.daily_limit_minutes_override is null;

-- ─── 3. expire_mynt reads the child's own expiry window ──────────────
...
-- ─── 4. redeem_screen_time reads the child's own cap + mission gate ──
...
begin
  ...
  -- Only the multiplier still comes from the family config; the daily
  -- cap is per-child now (60 min if somehow still unset).
